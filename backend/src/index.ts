...
const users: { name: string; email: string; passwordHash: string }[] = [];
...
app.post(
  ...
  async (req: Request, res: Response) => {
    ...
    const language = (req.body.language as string) || "as";
    ...
    try {
      ...
      const mediaId: string = submitRes.data.id;
      ...
      for (let i = 0; i < 30; i++) {
        ...
        const statusRes = await axios.get(
          `https://api.sonix.ai/v1/media/${mediaId}`,
        ...
        if (statusRes.data.status === "completed") {
          ...
          const textRes = await axios.get(
            `https://api.sonix.ai/v1/media/${mediaId}/transcript`,
            {
              headers: {
                Authorization: `Bearer ${SONIX_API_KEY}`,
                Accept: "text/plain",
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const ttsRes = await axios.post(
        "https://api.sonix.ai/v1/text_to_speech",
        payload,
        {
          headers: {
            Authorization: `Bearer ${SONIX_API_KEY}`,
